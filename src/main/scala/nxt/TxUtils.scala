...
object TxUtils {
  ...
  def issueTx(phrase: String, attachment: Attachment, amount: Long,
              recipientOpt: Option[Long], rcpPubKeyOpt: Option[Array[Byte]]): Try[Transaction] = Try {
  ...
  def issueTx(phrase: String, attachment: Attachment, amount: Long, recipientOpt: Option[Long]): Try[Transaction] =
    issueTx(phrase, attachment, amount, recipientOpt, None)
  ...
  def publicKeyAnnouncement(phrase:String) = {
    ...
    issueTx(phrase, Attachment.ORDINARY_PAYMENT, 1, Some(accId), Some(pk))
  }
